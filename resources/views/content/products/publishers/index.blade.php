...
<div class="app-ecommerce-publishers">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1"><i class="ti tabler-building ti-md me-1 text-primary"></i> Product Publishers</h4>
            <p class="text-muted mb-0">Manage game and software publishers</p>
        </div>
        <button class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasPublisherList">
            <i class="ti tabler-plus ti-xs me-1"></i> Add Publisher
        </button>
    </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Total Publishers</small>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="bulk-bar p-3 mb-3 d-none" id="bulk-bar">
        <div class="d-flex align-items-center justify-content-between">
            <span class="fw-semibold"><span id="bulk-count">0</span> publisher(s) selected</span>
    ...
    <div class="card shadow-sm">
        <div class="card-header border-bottom">
            <div class="d-flex flex-wrap justify-content-between align-items-center row-gap-2">
                <h5 class="card-title mb-0">All Publishers</h5>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" id="publishers-table" style="width:100%">
...
(function ($) {
    ...
    const Page = {
        ...
        initDataTable() {
            this.table = $('#publishers-table').DataTable({
                processing: true,
                serverSide: true,
                ...
                language: {
                    emptyTable: '<div class="py-4 text-center"><i class="ti tabler-building ti-xl text-muted mb-2 d-block"></i><span class="text-muted">No publishers found</span></div>'
                }
            });
        },
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: () => {
                            ...
                            Swal.fire({ icon: 'success', title: 'Publisher deleted', showConfirmButton: false, timer: 1500, timerProgressBar: true });
                        },
        ...
        bulkDelete() {
            ...
            Swal.fire({
                ...
                text: `${ids.length} publisher(s) will be permanently deleted.`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes, delete',
                customClass: { confirmButton: 'btn btn-danger me-3', cancelButton: 'btn btn-label-secondary' },
                ...
                if (!r.isConfirmed) return;
                $.post('{{ route('publishers.bulk-delete') }}', { ids, _token: '{{ csrf_token() }}' })
                    .done(() => {
                        this.table.ajax.reload(null, false);
                        $('#select-all').prop('checked', false);
                        this.syncBulkBar();
                        Swal.fire({ icon: 'success', title: 'Publishers deleted', showConfirmButton: false, timer: 1500, timerProgressBar: true });
                    })
