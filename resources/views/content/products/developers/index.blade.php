@extends('layouts.app')
@section('title', 'Product Developers')
...
@section('content')
<div class="app-ecommerce-developers">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1"><i class="ti tabler-code ti-md me-1 text-primary"></i> Product Developers</h4>
            <p class="text-muted mb-0">Manage game and software developers</p>
        </div>
        <button class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasDeveloperList">
            <i class="ti tabler-plus ti-xs me-1"></i> Add Developer
        </button>
    </div>
    ...
    <div class="bulk-bar p-3 mb-3 d-none" id="bulk-bar">
        <div class="d-flex align-items-center justify-content-between">
            <span class="fw-semibold"><span id="bulk-count">0</span> developer(s) selected</span>
    ...
    <div class="card shadow-sm">
        <div class="card-header border-bottom">
            <div class="d-flex flex-wrap justify-content-between align-items-center row-gap-2">
                <h5 class="card-title mb-0">All Developers</h5>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" id="developers-table" style="width:100%">
    ...
    <!-- Offcanvas Add -->
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasDeveloperList">
        <div class="offcanvas-header py-6">
            <h5 class="offcanvas-title">Add Developer</h5>
...
(function ($) {
    ...
    const Page = {
        ...
        initDataTable() {
            this.table = $('#developers-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('developers.index') }}',
                order: [[1, 'asc']],
                pageLength: 25,
                columns: [
                ...
                language: {
                    emptyTable: '<div class="py-4 text-center"><i class="ti tabler-code ti-xl text-muted mb-2 d-block"></i><span class="text-muted">No developers found</span></div>'
                }
            });
        },
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: () => {
                            ...
                            Swal.fire({ icon: 'success', title: 'Developer deleted', showConfirmButton: false, timer: 1500, timerProgressBar: true });
                        },
        ...
        bulkDelete() {
            ...
            Swal.fire({
                title: 'Delete Developers?',
                text: `${ids.length} developer(s) will be permanently deleted.`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes, delete',
                customClass: { confirmButton: 'btn btn-danger me-3', cancelButton: 'btn btn-label-secondary' },
                ...
                if (!r.isConfirmed) return;
                $.post('{{ route('developers.bulk-delete') }}', { ids, _token: '{{ csrf_token() }}' })
                    .done(() => {
                        this.table.ajax.reload(null, false);
                        $('#select-all').prop('checked', false);
                        this.syncBulkBar();
                        Swal.fire({ icon: 'success', title: 'Developers deleted', showConfirmButton: false, timer: 1500, timerProgressBar: true });
                    })
