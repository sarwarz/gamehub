    ...
    <form method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
            <div class="d-flex flex-column justify-content-center">
                ...
                <p class="mb-0">Update the product details below</p>
            </div>
        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-repeater" id="minimum-req">
                            <div data-repeater-list="system_requirements[minimum]">
                                @php
                                    $minReqs = old('system_requirements.minimum', $product->system_requirements['minimum'] ?? []);
                                @endphp

                                @forelse($minReqs as $req)
                                    <div data-repeater-item class="row mb-3 align-items-center">
                                        <div class="col-md-4">
                                            <input type="text" name="key" class="form-control" value="{{ $req['key'] ?? '' }}" readonly>
                                        </div>
                                        <div class="col-md-7">
                                            <input type="text" name="value" class="form-control" value="{{ $req['value'] ?? '' }}">
                                        </div>
                                        <div class="col-md-1">
                        ...
                        <div class="form-repeater" id="recommended-req">
                            <div data-repeater-list="system_requirements[recommended]">
                                @php
                                    $recReqs = old('system_requirements.recommended', $product->system_requirements['recommended'] ?? []);
                                @endphp

                                @forelse($recReqs as $req)
                                    <div data-repeater-item class="row mb-3 align-items-center">
                                        <div class="col-md-4"><input type="text" name="key" value="{{ $req['key'] ?? '' }}" class="form-control" readonly></div>
                                        <div class="col-md-7"><input type="text" name="value" value="{{ $req['value'] ?? '' }}" class="form-control"></div>
                                        <div class="col-md-1"><button data-repeater-delete type="button" class="btn btn-sm btn-danger">X</button></div>
                                    </div>
                                @empty
                                    <div class="form-repeater" id="recommended-req">
                        ...
                        <div class="form-repeater">
                            <div data-repeater-list="system_requirements[extra]">
                                @php
                                    $extraReqs = old('system_requirements.extra', $product->system_requirements['extra'] ?? []);
                                @endphp

                                @forelse($extraReqs as $req)
                                    <div data-repeater-item class="row mb-3 align-items-center">
                                        <div class="col-md-4"><input type="text" name="key" value="{{ $req['key'] ?? '' }}" class="form-control"></div>
                                        <div class="col-md-7"><input type="text" name="value" value="{{ $req['value'] ?? '' }}" class="form-control"></div>
                                        <div class="col-md-1"><button data-repeater-delete type="button" class="btn btn-sm btn-danger">X</button></div>
                                    </div>
                                @empty
                                    <div data-repeater-item class="row mb-3 align-items-center">
                                        <div class="col-md-4"><input type="text" name="key" class="form-control" placeholder="e.g. DirectX"></div>
                                        <div class="col-md-7"><input type="text" name="value" class="form-control" placeholder="DirectX 12"></div>
                                        <div class="col-md-1"><button data-repeater-delete type="button" class="btn btn-sm btn-danger">X</button></div>
                                    </div>
                                @endforelse
                            </div>
                    ...
                    <div class="card-body">
                        <div class="form-repeater">
                            <div data-repeater-list="attributes">
                                @php
                                    // Prefer old() if validation failed, otherwise use existing product attributes
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="meta_description" rows="3"
                                    ...
                                    placeholder="Enter SEO description (max 160 characters)">{{ old('meta_description', $product->meta_description ?? '') }}</textarea>
            ...
            <div class="col-12 col-lg-4">
                ...
                <!-- Category Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Category</h5></div>
                    <div class="card-body">
                        <select name="category_ids[]" class="form-select select2 @error('category_ids') is-invalid @enderror" multiple>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}"
                                    {{ in_array($cat->id, old('category_ids', $product->categories->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="card-body">
                        <select name="platform_ids[]" class="form-select select2 @error('platform_ids') is-invalid @enderror" multiple>
                            @foreach($platforms as $p)
                                <option value="{{ $p->id }}"
                                    {{ in_array($p->id, old('platform_ids', $product->platforms->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $p->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Product Type Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Product Type</h5></div>
                    <div class="card-body">
                        <select name="type_ids[]" class="form-select select2 @error('type_ids') is-invalid @enderror" multiple>
                            @foreach($types as $t)
                                <option value="{{ $t->id }}"
                                    {{ in_array($t->id, old('type_ids', $product->types->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $t->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Region Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Region</h5></div>
                    <div class="card-body">
                        <select name="region_ids[]" class="form-select select2 @error('region_ids') is-invalid @enderror" multiple>
                            @foreach($regions as $r)
                                <option value="{{ $r->id }}"
                                    {{ in_array($r->id, old('region_ids', $product->regions->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $r->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Language Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Language</h5></div>
                    <div class="card-body">
                        <select name="language_ids[]" class="form-select select2 @error('language_ids') is-invalid @enderror" multiple>
                            @foreach($languages as $lang)
                                <option value="{{ $lang->id }}"
                                    {{ in_array($lang->id, old('language_ids', $product->languages->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $lang->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Works On Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Works On</h5></div>
                    <div class="card-body">
                        <select name="works_on_ids[]" class="form-select select2 @error('works_on_ids') is-invalid @enderror" multiple>
                            @foreach($workson as $w)
                                <option value="{{ $w->id }}"
                                    {{ in_array($w->id, old('works_on_ids', $product->worksOn->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                    {{ $w->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Developer Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Developer</h5></div>
                    <div class="card-body">
                        <select name="developer_id" class="form-select select2 @error('developer_id') is-invalid @enderror">
                            <option value="">-- Select Developer --</option>
                            @foreach($developers as $d)
                                <option value="{{ $d->id }}"
                                    {{ (old('developer_id', $product->developer_id ?? '') == $d->id) ? 'selected' : '' }}>
                                    {{ $d->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <!-- Publisher Card -->
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Publisher</h5></div>
                    <div class="card-body">
                        <select name="publisher_id" class="form-select select2 @error('publisher_id') is-invalid @enderror">
                            <option value="">-- Select Publisher --</option>
                            @foreach($publishers as $pub)
                                <option value="{{ $pub->id }}"
                                    {{ (old('publisher_id', $product->publisher_id ?? '') == $pub->id) ? 'selected' : '' }}>
                                    {{ $pub->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="card mb-6">
                    ...
                    <div class="card-body">
                        <select name="delivery_type" class="form-select select2 @error('delivery_type') is-invalid @enderror">
                            @php
                                $delivery = old('delivery_type', $product->delivery_type ?? 'instant');
                            @endphp
                            <option value="instant" {{ $delivery == 'instant' ? 'selected' : '' }}>Instant</option>
                            <option value="manual" {{ $delivery == 'manual' ? 'selected' : '' }}>Manual</option>
                            <option value="email" {{ $delivery == 'email' ? 'selected' : '' }}>Email</option>
                            <option value="link" {{ $delivery == 'link' ? 'selected' : '' }}>External Link</option>
                        </select>
                ...
                <div class="card mb-6">
                    <div class="card-header"><h5 class="card-title mb-0">Status & Featured</h5></div>
                    <div class="card-body">
                        <div class="mb-3">
...
<script>
    $(document).ready(function () {
        ...
        let tagify = new Tagify(input, {
            // Split existing comma-separated keywords into tags
            delimiters: ",",
            originalInputValueFormat: valuesArr => valuesArr.map(item => item.value).join(",")
        });

        // If editing, convert the comma string to Tagify items
        let existing = input.value;
        if (existing && !existing.startsWith("[")) {
            let keywords = existing.split(',').map(v => ({ value: v.trim() })).filter(v => v.value);
            tagify.addTags(keywords);
        }

        // Ensure plain string before submit
